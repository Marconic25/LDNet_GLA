...
def run(mode,W0):
    ...
    if mode=='opt':
        ctrl=Controller(a.predict,U=U,dt=DT,Q_h=0,Q_alpha=0,Q_alpha_dot=3e5,Q_CL=1e3,R=0.3,delta_max=20.,delta_dot_max=100.,C_L_trim=CLt,Fy_trim=Fyt,Mz_trim=Mzt,global_search=True,n_grid=15,causal_basin=True,target_lpf=0.95,e_ref=0.8,lpf_max=0.995);ctrl.reset()
...
for W0 in [5,10,15,20,30]:
    o=run('open',W0)
    for nm,mode in [('prop','prop'),('opt-sched','opt')]:
        r=run(mode,W0)
        rc=(o[0]-r[0])/o[0]*100; rh=(o[1]-r[1])/o[1]*100; ra=(o[2]-r[2])/o[2]*100
        print('%4d %-10s | %+6.1f%%  %+7.1f%%  %+7.1f%%  %4.1f'%(W0,nm,rc,rh,ra,r[3]), flush=True)
