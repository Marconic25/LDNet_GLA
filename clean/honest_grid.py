"""HONEST single-step optimal (no gust oracle) vs proportional, full grid.

The optimal does NOT use the true gust W. It anchors the lift LEVEL to the MEASURED
C_L (same sensor as the proportional) and uses the model only for the SHAPE in delta,
evaluated at W=0 (the unknown-gust offset cancels in the difference):
    Chat(delta) = C_L_meas + [ model(x,delta,W=0) - model(x,delta_prev,W=0) ]
    delta* = argmin_delta (Chat(delta)-CL_trim)^2 + R*delta^2   (causal half, rate-limited)

Minimal 2-term objective; R tuned per cell to maximize CL reduction WITHOUT exciting
pitch beyond open-loop. R sweep is vectorized (batched over candidates). Saves
results/gridplots.npz (open/prop/opt full traces) for local plotting.
"""
import os, numpy as np
import mpc_gust as M
from controller import Controller

DT = M.DT; U = M.U; CLTRIM = M.CLTRIM; q = M.q; C = M.C
LAM = float(M.a._z_leak)
_dummy = Controller(aero_predict=M.a.predict, U=U, dt=DT)
_rk4b = _dummy._rk4_batch
TEND = float(os.environ.get('TEND', '3.0'))
GAINS = [-10., -20., -40., -60., -80., -120., -160.]
R_GRID = np.array([1e-2, 3e-3, 1e-3, 3e-4, 1e-4])
DMAX = 14.0; G = 161
GRID = {
    'W10/T0.5': (10., 0.5), 'W10/T1.0': (10., 1.0), 'W10/T2.0': (10., 2.0),
    'W20/T0.5': (20., 0.5), 'W20/T1.0': (20., 1.0), 'W20/T2.0': (20., 2.0),
    'W30/T0.5': (30., 0.5), 'W30/T1.0': (30., 1.0), 'W30/T2.0': (30., 2.0),
    'design'  : (11.46, 1.12),
}
QTY = ['h', 'al', 'ad', 'CL', 'CM', 'de']
LIMIT = ['W30/T0.5', 'W10/T2.0', 'W30/T2.0']   # gust corners (Tg=2.0 = out-of-envelope, flagged)


def clexc(CL, t, Tg):
    mw = t <= (Tg + 0.5)
    return float(np.max(np.abs(CL[mw] - CLTRIM)))


def batch_honest(W0, Tg):
    """Parallel HONEST single-step optimal for all R in R_GRID. No gust oracle:
    level from measured C_L, shape from the model at W=0. Returns t, Wt, per-R traces."""
    a = M.a; B = len(R_GRID)
    N = int(round(TEND / DT)) + 1
    t = np.arange(N) * DT; Wt = np.array([M.gust(tt, W0, Tg) for tt in t])
    a.reset(dt=DT)
    z_b = np.tile(np.asarray(a._z, float).reshape(1, -1), (B, 1))
    x_b = np.tile(np.asarray(M.X0, float).reshape(1, -1), (B, 1))
    prev = np.zeros(B); reach = 300.0 * DT
    dg = np.linspace(-DMAX, DMAX, G)
    rec = {k: np.zeros((N, B)) for k in QTY}
    for i in range(N):
        Wi = float(Wt[i])
        clm, _, _ = a.batch_step(z_b, x_b, prev, Wi, U, DT)                 # (B,) MEASURED C_L (true gust)
        base, _, _ = a.batch_step(z_b, x_b, prev, 0.0, U, DT)              # (B,) model C_L at prev, W=0
        xb_big = np.repeat(x_b, G, axis=0); zb_big = np.repeat(z_b, G, axis=0)
        CLg, _, _ = a.batch_step(zb_big, xb_big, np.tile(dg, B), 0.0, U, DT)  # (B*G,) model shape, W=0
        CLg = CLg.reshape(B, G)
        Chat = clm[:, None] + (CLg - base[:, None])                        # anchored prediction
        cost = (Chat - CLTRIM) ** 2 + R_GRID[:, None] * dg[None, :] ** 2
        neg = dg[None, :] <= 0.0
        causal = np.where(clm[:, None] >= CLTRIM, neg, ~neg)               # sign from MEASURED excursion
        ratem = np.abs(dg[None, :] - prev[:, None]) <= reach + 1e-9
        cost = np.where(causal & ratem, cost, np.inf)
        d = dg[np.argmin(cost, axis=1)]
        cl, cm, z_new = a.batch_step(z_b, x_b, d, Wi, U, DT)               # apply: true force + advance
        z_b = z_new - LAM * z_b
        x_b = _rk4b(x_b, q * cl, q * cm * C, DT)
        prev = d
        rec['h'][i] = x_b[:, 0]; rec['al'][i] = x_b[:, 2]; rec['ad'][i] = x_b[:, 3]
        rec['CL'][i] = cl; rec['CM'][i] = cm; rec['de'][i] = d
    return t, Wt, rec


def main():
    print(f'# honest_grid (no gust oracle, anchored to measured C_L)  TEND={TEND} R_GRID={list(R_GRID)} '
          f'G={G} DAMULT={os.environ.get("DAMULT","1")}', flush=True)
    sel = os.environ.get('CELLS', 'limit')
    names = LIMIT if sel == 'limit' else list(GRID.keys())
    out = {}
    for name in names:
        W0, Tg = GRID[name]
        tag = name.replace('/', '_').replace('.', '')
        OL = M.simulate('open', W0, Tg, TEND=TEND, DLPF=0.0)
        t = OL['_t']; cex0 = clexc(OL['CL'], t, Tg); mw = t <= (Tg + 0.5)
        pitch_open = float(np.max(np.abs(OL['al'][mw])))
        bp = (None, -1e9, None)
        for g in GAINS:
            r = M.simulate('prop', W0, Tg, gain=g, TEND=TEND, DLPF=0.0)
            cr = (cex0 - clexc(r['CL'], t, Tg)) / cex0 * 100
            if cr > bp[1]: bp = (g, cr, r)
        gstar, pcr, PR = bp
        _, W, rec = batch_honest(W0, Tg)
        crs = np.array([(cex0 - clexc(rec['CL'][:, j], t, Tg)) / cex0 * 100 for j in range(len(R_GRID))])
        prs = np.array([float(np.max(np.abs(rec['al'][mw, j]))) / max(pitch_open, 1e-12) for j in range(len(R_GRID))])
        ok = np.where(prs <= 1.10)[0]
        jb = int(ok[np.argmax(crs[ok])]) if len(ok) else int(np.argmin(prs))
        Rstar = float(R_GRID[jb]); ocr = float(crs[jb]); opr = float(prs[jb])
        for arm, r in [('open', OL), ('prop', PR)]:
            for k in QTY:
                out[f'{tag}_{arm}_{k}'] = r[k]
        for k in QTY:
            out[f'{tag}_opt_{k}'] = rec[k][:, jb]
        out[f'{tag}_t'] = t; out[f'{tag}_W'] = W
        out[f'{tag}_gstar'] = float(gstar); out[f'{tag}_Rstar'] = Rstar; out[f'{tag}_Tg'] = float(Tg)
        out[f'{tag}_pcr'] = float(pcr); out[f'{tag}_ocr'] = ocr
        print(f'{name:10s}: prop g*={gstar:6.0f} CLred={pcr:5.1f}% pitch=1.00 | '
              f'opt(honest) R*={Rstar:8.0e} CLred={ocr:5.1f}% pitch={opr:.2f}', flush=True)
    import os as _o; _o.makedirs('results', exist_ok=True)
    np.savez('results/gridplots.npz', **out)
    print('# saved results/gridplots.npz', flush=True)
    print('# DONE', flush=True)


if __name__ == '__main__':
    main()
