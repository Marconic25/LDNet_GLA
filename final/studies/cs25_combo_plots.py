"""
Forked from light/tests/cs25_combo_plots.py for the v6 campaign. Not in the
task's file list verbatim, but included because it is what actually produces
the per-cell table (summary.md) that tab:cs25_grid is transcribed from —
cs25_combo_study.py alone only writes the raw traces_W*.npz. Same relationship
as noise_white_combo.py + "its plotter" elsewhere in this folder.

Derivation — what changed vs the light/ original: DIR_COMBO now resolves via
paths.results_dir('cs25_combo') (-> final/results_cs25_combo) instead of
light/results_cs25_combo/. Everything else (heatmap, line plot, summary.md
table format) is verbatim. These are diagnostic plots (heatmap/line titles),
not the AGENTS.md-styled thesis figures — those are cs25_thesis_figs.py,
forked separately.

CS-25 plots for the E2-combo controller.

Reads:
  final/results_cs25_combo/traces_W{10,20,30}.npz (combo)

Generates in final/results_cs25_combo/:
  heatmap_clred_combo.png   – CLred heatmap (combo)
  summary_lines_combo.png   – CLred vs Tg, one line per W0
  summary.md                – per-cell table (source for tab:cs25_grid)
...
DIR_COMBO = paths.results_dir('cs25_combo')
...
fig.suptitle('CS-25.341 – CLred (DAMULT=3, R* per cell, no-explosion pick) [v6]', fontsize=9)
plt.tight_layout()
...
ax.set_title('CS-25 – E2-combo [v6]')
