"""
Forked from light/tests/cs25_combo_study.py for the v6 (dataset_v6) campaign.
Derivation — what changed vs the light/ original:
  1. `sys.path.insert(...)` + `import run as Rn` -> `import paths` (puts
     light/ on sys.path) + `import run_v6 as Rn` (points at the v6 model via
     MD_OVERRIDE; see run_v6.py — light/run.py itself is untouched).
  2. OUT_DIR: RESULTS_DIR_OVERRIDE now defaults to final/results_cs25_combo
     (paths.results_dir('cs25_combo')) instead of light/results_cs25_combo/,
     so the v5 archive is never touched. RESULTS_DIR_OVERRIDE still works
     for an explicit override, exactly as before.
Everything else (TG_LIST, W0_LIST, R_GRID, the per-cell loop, the pick rule,
the npz schema) is verbatim — the 3 W0 x 6 Tg x 5 R CS-25 grid backing
tab:cs25_grid.
...
OUT_DIR = os.environ.get('RESULTS_DIR_OVERRIDE', paths.results_dir('cs25_combo'))
os.makedirs(OUT_DIR, exist_ok=True)

print(f'# cs25_combo_study [v6] MODE={MODE} NH={NH}  W0_LIST={W0_LIST}  '
      f'TG_LIST={TG_LIST}  R_GRID={R_GRID}  TEND={TEND}  '
      f'DAMULT={os.environ.get("DAMULT", "1")}  MD={Rn.MD}', flush=True)
