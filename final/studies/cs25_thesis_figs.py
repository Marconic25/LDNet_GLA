"""
Forked from light/tests/cs25_thesis_figs.py for the v6 campaign.
Derivation — what changed vs the light/ original:
  1. DIR (input traces) and IMG_DIR (output PNGs) now come from paths.py:
     DIR = final/results_cs25_combo (paths.results_dir('cs25_combo')),
     IMG_DIR = final/images (paths.img_dir()) — NOT light/latex/Images/,
     which still backs the current (v5) thesis text.
  2. FOM_CSV defaults to {} instead of the four hardcoded v5 cluster paths
     under /work/u10677113/NACA2312/mpc_fom_dagger/...: no v6 FOM
     verification runs exist yet. It can be repopulated once they do, either
     by editing this dict or by pointing LDNET_V6_FOM_CSV_JSON at a JSON file
     mapping "W0,Tg" -> csv path (see load_fom_csv() below).
Everything else — the gust cells, the trace/envelope figure layout, the
AGENTS.md plot style (serif + cm mathtext, ~9pt, no titles, 300 dpi, no
bbox_inches='tight' on paired subfloats) — is verbatim.

Thesis figures (chapter 3) for the noise-free CS-25 parametric grid.

Reads final/results_cs25_combo/traces_W{10,20,30}.npz and generates, in the
same folder and in the v6 image directory:
...
Style follows light/latex/AGENTS.md ("Linee guida per i plot"): serif + cm
mathtext, ~9 pt at print width, no in-figure titles, 300 dpi.
...
run, independent of the ROM used everywhere else in this script.
...
DMAX    = 14.0
DIR     = paths.results_dir('cs25_combo')
...
# real OpenFOAM + structural physics. Empty by default: no v6 FOM
# verification runs exist yet (light/tests/cs25_thesis_figs.py's FOM_CSV
# pointed at v5 cluster runs under mpc_fom_dagger/, not reusable here).
# Repopulate by editing this dict, or by pointing LDNET_V6_FOM_CSV_JSON at a
# JSON file of the form {"W0,Tg": "path/to/structural_trajectory.csv"}.
FOM_CSV = {}
_fom_json = os.environ.get('LDNET_V6_FOM_CSV_JSON')
if _fom_json and os.path.exists(_fom_json):
    with open(_fom_json) as f:
        _raw = json.load(f)
    FOM_CSV = {tuple(float(x) for x in k.split(',')): v for k, v in _raw.items()}
...
def save(fig, name, tight=True):
    ...
    print(f'saved {fn} (+ {IMG_DIR})', flush=True)
