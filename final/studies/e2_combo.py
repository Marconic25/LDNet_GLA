"""
Forked from light/noise/e2_combo.py for the v6 campaign.
Derivation — what changed vs the light/ original:
  1. `import harness_noise as H` -> `import paths` + `H = paths.load_v6_harness()`
     (see paths.py docstring: harness_noise.py is imported UNCHANGED, then its
     module-level aero/CLTRIM/LAM are monkey-patched at the v6 model, because
     it hardcodes its model path with no MD_OVERRIDE-style hook).
  2. `from controllers_ref import MPCConstRef` / `rk4_batch` / `dp45_batch`
     now resolve via paths.py's sys.path insertion of light/noise/ instead of
     relying on the script's own directory being on sys.path[0] (true when
     run directly, which is how these are always invoked, but made explicit
     here since this file no longer lives in light/noise/).
  3. OUT: final/results_noise/E2_combo_<part>.npz instead of light/noise/results/.
Everything else (FusedSensor, MPCPrevRdu, sigma models, the flat/dlr parts,
the best-combo trajectory dump) is verbatim.

Axis E2-combo — the literature pipeline COMPOSED (light/noise/NOTES.md, E2
verdict #5): T1 DLR massed-measurement fusion -> T7 N-step preview-horizon
MPC -> R_du move suppression for the residual acceleration flags.

Parts (run as two concurrent jobs; both share the script):
    python3 e2_combo.py flat   anchors (sigma=1e-6, R_du in {0, max}) +
                               fused flat-sigma frac=0.02, lam=0,
                               R_du in {0, 1e-2, 1e-1}
...
~N-times less against tracking than in a single-term cost; hence the sweep
{0, 1e-2, 1e-1}.
...
Output: final/results_noise/E2_combo_<part>.npz
"""
import os
import sys
import numpy as np
...
OUT = os.path.join(paths.results_dir('noise'), f'E2_combo_{PART}.npz')
...
class FusedSensor:
    ...
    of nodes i+1 .. i+N in self.last for the MPC. wc_fun returns node i+1
    (the scalar the harness logs as r['_Wc']). Honesty rule: Wt[m] is read
...
class MPCPrevRdu(MPCConstRef):
    """
    MPCConstRef + a move-suppression term R_du*(dg - prev)^2 on the single
    horizon move (constant-flap MPC: one move per step). Horizon step k is
    evaluated at sensor.last[k] = fused estimate of W(t+(k+1)*dt) — the
    wnext convention over the horizon. compute() ignores the scalar Wc
    harness argument.
    """
...
print(f"# axis E2-combo [v6/{PART}] N={NH} | W30/Tg0.4 DAMULT="
...
if PART == 'flat':
    ...
    print("\n=== anchors: sigma=1e-6 through fusion+MPC "
          f"(mpcp N=8 white-clean anchor) ===", flush=True)
