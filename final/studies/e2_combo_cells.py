"""
Forked from light/noise/e2_combo_cells.py for the v6 campaign.
Derivation — what changed vs the light/ original:
  1. `import harness_noise as H` -> `import paths` + `H = paths.load_v6_harness()`
     (see paths.py docstring / e2_combo.py header for the same change there).
  2. `from controllers_ref import MPCConstRef` / `rk4_batch` / `dp45_batch`
     resolve via paths.py's sys.path insertion of light/noise/.
  3. OUT: final/results_noise/E2_combo_cells_<cell>.npz instead of
     light/noise/results/.
Everything else (FusedSensor, MPCPrevRdu, cell selection, arms) is verbatim.

Axis E2CC — generality check of the composed pipeline on the other two cells.

e2_combo.py established the winner at the home cell W30/Tg0.4 (DAMULT=3):
...
R_du=0). This script repeats THE WINNER (fixed N=8, Jmax=50, R=3e-4, R_du=0
— no sweeps) at the two other study cells:
...
Output: final/results_noise/E2_combo_cells_<cell>.npz
...
OUT = os.path.join(paths.results_dir('noise'), f'E2_combo_cells_{cellname}.npz')
...
print(f"# axis E2-combo-cells [v6/{cellname}] N={NH} | W{W0:g}/Tg{Tg:g} DAMULT="
...
print("\n=== anchor: sigma=1e-6 through fusion+MPC "
      "(cell's clean combo reference) ===", flush=True)
