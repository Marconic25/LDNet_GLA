"""
Forked from light/noise/noise_timing_combo.py for the v6 campaign.
Derivation — what changed vs the light/ original:
  1. `sys.path.insert(...)` + `import harness_noise as H` ->
     `import paths` + `H = paths.load_v6_harness()` (see paths.py docstring).
  2. OUT: final/results_noise/B2_timing.npz instead of light/noise/results/.
Everything else (shift/refit sweeps, RefitSensor, npz schema) is verbatim.
...
Refit (sigma=2%*W0, 6 seeds): K in {1,5,10,25,42}
  K=1 == FusedPreviewSensor exactly (same rng stream).

Config: Jmax=50, lam=0, N=8, R=3e-4, R_du=0, home cell W30/Tg0.4, DAMULT=3.
Output: final/results_noise/B2_timing.npz
...
OUT = os.path.join(paths.results_dir('noise'), 'B2_timing.npz')
...
print(f"# B2_timing [v6] | W{W0:g}/Tg{Tg:g} DAMULT={os.environ.get('DAMULT','1')} "
