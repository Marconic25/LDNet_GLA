"""
Forked from light/noise/noise_white_combo.py for the v6 campaign.
Derivation — what changed vs the light/ original:
  1. `sys.path.insert(...)` + `import harness_noise as H` ->
     `import paths` + `H = paths.load_v6_harness()`, which imports
     harness_noise.py UNCHANGED and retargets its module-level `aero`/
     `CLTRIM`/`LAM` at the v6 model (see paths.py docstring for why this is
     a monkey-patch rather than a fork: harness_noise.py hardcodes its model
     path with no env-var hook, unlike light/run.py's MD_OVERRIDE).
  2. OUT: final/results_noise/W_combo.npz instead of light/noise/results/.
Everything else (sigma sweep, seeds, FusedPreviewSensor/MPCPreviewController
composition, the npz record schema) is verbatim.
...
Output: final/results_noise/W_combo.npz
...
OUT = os.path.join(paths.results_dir('noise'), 'W_combo.npz')
...
print(f"# W_combo [v6] | W{W0:g}/Tg{Tg:g} DAMULT={os.environ.get('DAMULT','1')} "
