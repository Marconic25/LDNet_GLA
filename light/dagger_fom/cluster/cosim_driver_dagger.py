...
def start_mpc_server(model, R, N, damult, dt, U=80.0, qadot=0.0, variant='qadot',
                     Qh=1e4, Qalpha=1e4, QCL=1e3):
    """Spawn the persistent MPC controller server inside the TF container.

    dagger_fom variant: launches mpc_fom_server_variant.py (Q_alpha_dot bolted
    onto light/optimal.py's cost, this study's first attempt) or
    mpc_fom_server_stable.py (full validated-stable cost ported from
    clean/controller.py: Q_h, Q_alpha, Q_alpha_dot, Q_CL, R -- see
    light/dagger_fom/controller_stable.py) instead of light/tests'
    mpc_fom_server.py, selected by `variant`.
    """
    global _MPC_PROC
    ...
    if variant == 'stable':
        script = ("mpc_fom_server_stable.py "
                  f"--model {model} --R {R} --N {N} --damult {damult} --dt {dt} --U {U} "
                  f"--Qh {Qh} --Qalpha {Qalpha} --Qadot {qadot} --QCL {QCL}")
    else:
        script = ("mpc_fom_server_variant.py "
                  f"--model {model} --R {R} --N {N} --damult {damult} --dt {dt} --U {U} --qadot {qadot}")
    cmd = APPTAINER_CMD_TF + [
        "/bin/bash", "-c",
        f"cd {LDNET_GLA_DIR}/light/dagger_fom && "
        f"OMP_NUM_THREADS=1 TF_NUM_INTRAOP_THREADS=1 TF_NUM_INTEROP_THREADS=1 "
        f"python3 -u {script}"
    ]
    print(f"  Starting MPC server ({variant}): model={model} R={R} N={N} damult={damult} dt={dt}")
...
def main():
    ...
    parser.add_argument("--qadot", type=float, default=0.0,
                        help="dagger_fom study: pitch-rate move-suppression weight "
                             "for MPCPreviewControllerQadot (0 = same as light/optimal.py's "
                             "MPCPreviewController).")
    parser.add_argument("--controller-variant", choices=["qadot", "stable"], default="qadot",
                        help="dagger_fom study: 'qadot' = Q_alpha_dot bolted onto light/optimal.py's "
                             "cost (mismatched R/Q_CL scale, first attempt). 'stable' = full "
                             "validated-stable cost ported from clean/controller.py "
                             "(Q_h, Q_alpha, Q_alpha_dot, Q_CL, R -- see controller_stable.py). "
                             "In 'stable' mode --mpc-R is IGNORED (use --R instead, default 1.0).")
    parser.add_argument("--Qh", type=float, default=1e4, help="stable variant: heave cost weight")
    parser.add_argument("--Qalpha", type=float, default=1e4, help="stable variant: pitch-angle cost weight")
    parser.add_argument("--QCL", type=float, default=1e3, help="stable variant: C_L tracking cost weight")
    parser.add_argument("--R", type=float, default=1.0, help="stable variant: flap-effort cost weight")
    ...
    if _mpc_server_needed:
        if args.mpc_model is None or (args.mpc_R is None and args.controller_variant != "stable"):
            raise ValueError("--controller mpc (or --also-spawn-mpc-idle) requires --mpc-model and --mpc-R "
                             "(--mpc-R not needed for --controller-variant stable, uses --R instead)")
        if CONTROLLER == "mpc":
            D_ALPHA = D_ALPHA * args.damult   # match the real wing to the controller's assumption
        _server_R = args.R if args.controller_variant == "stable" else args.mpc_R
        start_mpc_server(args.mpc_model, _server_R, args.mpc_N, args.damult, _MPC_CTRL_DT,
                         qadot=args.qadot, variant=args.controller_variant,
                         Qh=args.Qh, Qalpha=args.Qalpha, QCL=args.QCL)
        mpc_reset()
    ...
    while t_cur < t_end - 1e-12:
        ...
        # Purge old processor time dirs: only latestTime is needed to chain the
        # next window; without this, one dir per window accumulates (~8 MB each)
        # and multi-thousand-window runs blow the disk quota.
        for proc in sorted(CASE_DIR.glob("processor*")):
            numeric = []
            for d in proc.iterdir():
