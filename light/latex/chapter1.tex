...
The geometry considered is a 2D airfoil with a flap attached as shown in \cref{fig:airfoil_scheme}: the structural model has two dynamic degrees of freedom, the plunge $h$ and the pitch $\alpha$. The plunge $h$, positive downward, measures the vertical translation of the elastic axis, whose reference position is $\mathbf{x}_{\mathrm{EA}}$, while the pitch is the rotation about that axis, positive nose-up. The trailing-edge flap is hinged to the airfoil through a rigid hinge, but its rotation $\delta$ is imposed in time as the deflection chosen by the control law. From the point of view of the structural model the flap is therefore a kinematic input, not a degree of freedom, and its dynamics is reduced to the schedule $\delta(t)$. The vector of generalised coordinates is $\mathbf{q} = (h(t),\,\alpha(t))^{T}$: $\delta(t)$ enters the equations as an external excitation.
...
&& \text{in } (0,T],
\label{eq:eom}\\
&F_y(t) = \int_{\Gamma_I(t)} \bigl[\bigl(-\rho\,p\,\mathbf{I} + \boldsymbol{\tau}\bigr)\cdot\mathbf{n}\bigr]\cdot\hat{\mathbf e}_y \,\mathrm{d}\Gamma,
&& \text{in } (0,T],
\label{eq:interface-loads}\\
&M_z(t) = \int_{\Gamma_I(t)} \Bigl\{(\mathbf{x}-\mathbf{x}_{\mathrm{EA}})\times\bigl[\bigl(-\rho\,p\,\mathbf{I} + \boldsymbol{\tau}\bigr)\cdot\mathbf{n}\bigr]\Bigr\}\cdot\hat{\mathbf e}_z \,\mathrm{d}\Gamma,
&& \text{in } (0,T].
...
\begin{figure}[t]
    ...
    airfoil carries a trailing-edge flap on a rigid hinge and is subject to the
...
The parameters $m_w$ and $m_f$ are the wing and flap masses, $I_w$ the wing moment of inertia about the elastic axis, $I_{f,\mathrm{EA}}$ and $I_{f,\mathrm{hinge}}$ the flap moments of inertia about the elastic axis and about the hinge, $d_x$ and $d_y$ the chordwise and vertical offsets of the hinge from the elastic axis, both shown in \cref{fig:airfoil_scheme}, $d_h$ and $d_\alpha$ the structural damping coefficients and $k_h$ and $k_\alpha$ the stiffnesses of the plunge and pitch degrees of freedom; the subset of these values used directly by the controller is listed in \cref{tab:results_setup} of \cref{app:fom}. Because the flap rotation is prescribed rather than solved for, the flap inertia enters the wing equations twice: through the off-diagonal terms of $\mathbf{M}$ and through $\mathbf{f}_\delta$, which collects the inertial and Coriolis contributions of the imposed motion.
...
\end{equation}
with $c_{\mathrm{ref}}$ the chord, $S_{\mathrm{ref}}$ the reference surface: the LDNet is therefore the model of the aerodynamic coefficients $C_L$ and $C_M$ of the airfoil, as functions of the structural state, the flap deflection and the gust, and it is queried at this single point throughout the present work, since $(F_y,M_z)$ is all the control loop requires. The same reconstruction network, queried instead at a set of points, returns the distributed flow field~\cite{Regazzoni}; this capability is not used by the controller and is examined separately, as a further probe of the model's capabilities, in \cref{subsubsec:field_recon}.

The parameter $U_\infty$ is carried by the formulation as an input parameter, but it is held
at $\SI{80}{\meter\per\second}$ in every trajectory of the simulation campaign of
\cref{subsubsec:campaign}: the LDNet has therefore seen it as a constant, and the operating
condition it represents is not one over which this work demonstrates generalisation. The
quantities the LDNet is trained to generalise over are the gust parameters $(W_0, T_g)$ and
the flap history, sampled over the intervals of \cref{tab:dataset_families}.
...
The control objective is to reduce the peak lift excursion that the airfoil experiences under the discrete ``$1-\cos$'' gusts of \labelcref{eq:gust}, over the gust amplitudes and gradient lengths of the envelope that \mbox{CS~25.341} prescribes for the design of the airframe~\cite{EASA_CS25}, which is the test bench on which the controller is assessed in \cref{subsec:mpc_results}. Since the vertical gust induces a pronounced peak in the lift coefficient resulting in increased bending moment at the root, in order to simplify the analysis the objective function was defined to penalise only the lift deviation $\Delta C_L = C_L - C_{L,\text{trim}}$, where $C_{L,\text{trim}}$ is the steady lift coefficient of the trimmed airfoil before the gust arrives, and the control effort $\delta$. Pitch is not directly penalised, and, as \cref{subsec:mpc_results} shows, it can become the binding constraint at the long-gust end of the envelope, with closed-loop pitch excursions up to $1.9$ times the open-loop ones. The control law is a Model Predictive Controller (MPC)~\cite{rawlings2017mpc,giesseler2012mpc} that uses the reduced model \labelcref{eq:reduced_system} as its internal model: at each sampling instant $t$ it solves the receding-horizon problem
