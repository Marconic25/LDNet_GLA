...
To integrate the structural equation of motion \labelcref{eq:eom} in time, we recast it in first-order form in the structural state $\boldsymbol{\zeta}$ of \cref{subsec:controller}, so that the state matrix $\mathbf{A}$ is $4\times4$ and the input matrix $\mathbf{B}$ is $4\times2$. The inversion of $\mathbf{M}$ is analytical and always defined, since the mass matrix \labelcref{eq:matrices} is constant, symmetric and positive definite, $\det\mathbf{M}=(m_w+m_f)(I_w+I_{f,\mathrm{EA}})-(m_f d_x)^2>0$. The form is not linear time-invariant: $\mathbf{f}_{\mathrm{aero}} = (-F_y,\,M_z)^T$ of \labelcref{eq:forcing_vectors} is supplied by the LDNet, with $F_y$ and $M_z$ the sectional loads of \cref{subsec:ldnet_arch}, and $\mathbf{f}_{\delta}$ carries the Coriolis term $-2\,m_f d_x\,\dot\alpha\,\dot\delta$ of the same equation, which depends on the state itself, with $\mathbf{0}$ and $\mathbf{I}$ the $2\times2$ zero and identity blocks below:
...
\begin{table}[t]
    ...
    \hline
    Gust peak $W_0$                                       & \SIrange{8}{48}{\meter\per\second}      & ---            & \SIrange{8}{48}{\meter\per\second} \T\B \\
    \hline
    Gust amplitude $r_g$                                  & 0.10--0.60 & ---            & 0.10--0.60 \T\B \\
    \hline
    Gust duration $T_g$                                   & \SIrange{0.30}{1.20}{\second} & ---            & \SIrange{0.30}{1.20}{\second} \T\B \\
    \hline
    Flap peak $\delta_{\mathrm{pk}}$                      & ---        & $\pm\SIrange{2}{15}{\degree}$  & \SIrange{-9.2}{-0.5}{\degree} \T\B \\
    \hline
    Rate limit $\dot\delta_{\mathrm{lim}}$                & ---        & \SIrange{20}{200}{\degree\per\second}        & \SIrange{20}{200}{\degree\per\second} \T\B \\
    \hline
    Gain $K_{\mathrm{ff}}$                                & ---        & ---            & \SIrange{0.05}{0.20}{\degree\second\per\meter} \T\B \\
    \hline
    Lag $t_d$                                             & ---        & ---            & \SIrange{0}{0.30}{\second} \T\B \\
    \hline
    Deflection range $\delta$                             & \SI{0}{\degree}        & \SIrange{-14.1}{14.8}{\degree} & \SIrange{-8.5}{0}{\degree} \T\B \\
    \hline
    \end{tabular}
    \\[6pt]
    ...
    $-\min(K_{\mathrm{ff}}\,W_0,\,\SI{20}{\degree})$, never above $\SI{9.6}{\degree}$ in magnitude
    over the sampled $K_{\mathrm{ff}}$ and $W_0$ and so never actually saturating, with
    $K_{\mathrm{ff}}$ the gain and $t_d$ the lag of
...
Every scalar input and output---the input signals ($h$, $\dot{h}$, $\alpha$, $\dot{\alpha}$, $\delta$, $W$), the parameter $U_\infty$, the loads ($F_y$, $M_z$) and the spatial query coordinate---is affinely mapped to the approximate interval $[-1,1]$ by the same min-max rule; for every quantity except $U_\infty$ the bounds are the dataset's own min and max, since the campaign of \cref{subsubsec:campaign} samples a range for each of them. $U_\infty$, instead, is held at the same $\SI{80}{\meter\per\second}$ in every trajectory, so its own min and max coincide, and the map is instead taken over a fixed physical range, $[0,\,\SI{120}{\meter\per\second}]$, chosen independently of the campaign. The time step is scaled by the reference time constant $\Delta t_{\mathrm{ref}} = \SI{0.002}{\second}$ before entering $\mathcal{NN}_\mathrm{dyn}$; the latent states $\mathbf{s}$ are not pre-normalised.
...
the autonomous-rollout comparison that motivates keeping both. Before proceeding with the discussion, it is worth specifying that the superscript $\mathrm{data}$ labels quantities drawn
...
state $\boldsymbol{\zeta}_n^\mathrm{data} = (h_n,\allowbreak\,\dot{h}_n,\allowbreak\,\alpha_n,\allowbreak\,\dot{\alpha}_n)^\mathsf{T}$,
...
feeds back as the network input at the next step and, once produced, is itself compared to
$\boldsymbol{\zeta}_{n+1}^\mathrm{data}$ inside $\mathcal{L}_\mathrm{rollout}$, alongside the load error at the matching index. The gradient
...
\begin{tikzpicture}[
  ...
  looparr/.style = {-stealth, semithick, cyan!60!black, thick},
]

%%────────── LEFT PANEL – Teacher forcing ────────────────────
\node[font=\small\bfseries] at (0.0, 5.55) {Teacher forcing};

\node[dat]   (xk)   at (-1.05, 4.15) {$\boldsymbol{\zeta}_n^{\mathrm{data}}$};
...
loads drive the structural integrator (loop), which feeds the rollout state back as
the next-step input; that state is in turn compared to the data inside the loss $\mathcal{L}_\mathrm{rollout}$.}
\label{fig:tf_vs_rollout}
\end{figure}

We therefore formulate a single loss functional in rollout form, of which the teacher-forcing
loss $\mathcal{L}_\mathrm{tf}$ used in the first stage is the special case given below: the 2-DOF
structural equation of motion is integrated from the reconstruction network's own predicted
loads $(F_y,\,M_z)$ over a horizon of $N_\mathrm{rollout} = 800$ steps of $\Delta t_{\mathrm{ref}}$,
i.e.\ \SI{1.6}{\second}, shorter than the \SI{3}{\second} runs of the dataset and than the
$T_g+\SI{0.5}{\second}$ evaluation window of \labelcref{eq:clred} at the longest gust duration,
with $\delta$ and $W$
treated as exogenous signals taken from the data. The aerodynamic outputs $F_y$ and $M_z$ are
...
\end{equation}
with $y_\mathrm{norm}$ a reference magnitude, set to $1$ throughout this work since $\tilde{\mathbf y}$ and $\mathbf y$ are already the min-max-normalised network outputs of \cref{subsubsec:campaign}, and $\epsilon = 10^{-4}$ to prevent division
by zero. The first term controls magnitude accuracy; the second penalises angular deviation,
relevant when $\tilde{\mathbf y}$ is a genuine multi-component vector, evaluated here on the
stacked $(F_y,M_z)$ pair with $w_\mathrm{dir}=0$, which recovers the standard
...
on a common dimensionless scale. The rollout structural state
...
Two architectural choices encode prior information about the aeroelastic system: the tail-expanding output layer $g$ of \cref{subsubsec:rollout}, whose larger gradient at the extremes (relative to the trim condition) biases the network towards accurate prediction of load extremes, and the exponential decay term of the latent update~\eqref{eq:latent_dyn}. That bias carries a mirror cost near the trim condition, where $g'(0) = \beta/(1+\beta) \approx 1/21$: at initialisation, with the outputs still close to zero, every gradient returned from the loss to the weights is attenuated by that factor, which slows the opening phase of the training without determining its outcome. The damped formulation biases the latent trajectory towards the
origin, encoding the prior that the system possesses a stable aeroelastic equilibrium: without
the decay term ($\lambda = 0$) the latent update has no restoring term of its own, and nothing
in the training objective prevents it from drifting unboundedly during autonomous propagation. The approach differs from the exact equilibrium constraint of
...
are first found by minimising the teacher-forcing loss $\mathcal{L}_\mathrm{tf}$, then
refined by minimising the rollout loss $\mathcal{L}_\mathrm{rollout}$~\eqref{eq:rollout_loss}
warm-started from those weights, as anticipated above. The remainder of this section derives
the gradient of $\mathcal{L}_\mathrm{rollout}$, the more involved of the two since it must be
propagated back through the structural integrator; the teacher-forcing gradient is the special
case in which that path is absent, as noted below. Defining
...
on the predicted loads. Because that same load prediction is $\mathcal{NN}_\mathrm{rec}(\mathbf{s}_n,\boldsymbol{\eta}_n;\mathbf{w}_\mathrm{rec})$, the gradient with respect to $\mathbf{w}_\mathrm{rec}$
under rollout training is carried by the same adjoint recursion~\eqref{eq:bptt} as $\mathbf{w}_\mathrm{dyn}$, through a different, more local block of the Jacobian; only under teacher forcing, where that block vanishes, does it reduce to standard back-propagation without temporal unrolling.

On $\mathcal{L}_\mathrm{tf}$, the weights are updated in two successive stages, both standard
optimisers used in their reference form. The first stage applies Adam~\cite{kingma2015adam},
whose per-weight adaptive rescaling of the gradient tolerates a large global learning rate,
$\eta = 10^{-2}$, and moves the networks away from poor initialisations over several
hundred epochs. The second stage switches to BFGS,
...
accuracy. The subsequent refinement on $\mathcal{L}_\mathrm{rollout}$, warm-started from
these weights, skips the Adam stage and runs BFGS directly, as detailed in
