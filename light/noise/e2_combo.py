...
This script composes them: the fusion database (rolling inverse-variance
sums, incl. the pre-warm) delivers the fused N-node preview VECTOR to the
MPC via the e2_mpc.PreviewSensor coupling (harness calls wc_fun BEFORE
ctrl.compute each step), and the MPC cost gets an R_du move-suppression
term on its single horizon move (constant-flap MPC).
