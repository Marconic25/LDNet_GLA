...
# Add src/ to path so utils/optimization are importable. This is a DEPTH-SWEEP copy of
# sensitivity_latent.py (src/ untouched per guardrail). The canonical loads ecosystem
# (utils, optimization with restore_best, GLA data) lives in the LDNet_GLA tree on the
# cluster, NOT NACA2312/src (which is a stale optimization.py). Honor SRC_OVERRIDE; else
# fall back to the repo src/ three levels up (correct locally).
import os
SRC_DIR = Path(os.environ.get("SRC_OVERRIDE",
...
dt              = 0.002
dt_base         = 0.002
num_epochs_Adam = int(os.environ.get("ADAM", "200"))
num_epochs_BFGS = int(os.environ.get("BFGS", "20000"))
# DEPTH-SWEEP knobs (defaults = original 2x7 dyn / 4x24 rec, bit-identical to the
# loads latent sweep). SEED_BASE varies the RNG family for replication error bars.
...
def build_networks(num_latent_states):
    ...
    # 2x7 / 4x24 reproduce the original architecture and RNG draw order exactly.
