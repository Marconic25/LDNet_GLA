...
% ================================================================
% Notation conventions follow chapter_lqr.tex.
% Macros assumed: \mA, \mB, \mC, \mK, \mP, \mQ, \mR, \vxi, \vy
% Cross-references: \ref{sec:lqr_arch}, \ref{sec:ekf}, \ref{sec:model},
%                   \ref{sec:jacobian}, \ref{sec:lqr}
% ================================================================

\subsection{Closed-Loop Results with MPC}

The LQR controller derived in the previous section solves an
\emph{infinite-horizon} optimisation problem \emph{once}, offline, on the
linearised plant.
The result is a fixed gain matrix $\mK$ that is applied as the static law
$\delta_k = -\mK\hat{\tilde{\vxi}}_k$ at every timestep: the flap command
depends only on the \emph{current} state estimate and is otherwise memoryless.
Two limitations follow directly from this design choice.
First, $\mK$ is computed on the linearised dynamics, so its optimality
degrades when the trajectory deviates far from trim.
Second, and more fundamentally, the LQR gain cannot look ahead: if the current
state is near zero but a large gust is about to arrive, the LQR issues almost
no command---it will react only once the state has grown large enough to produce
a significant term $-\mK\hat{\tilde{\vxi}}_k$.

Model Predictive Control (MPC) addresses both limitations by moving the
optimisation \emph{online}.
At each timestep $k$ the controller solves a \emph{finite-horizon} problem over
a sequence of $N$ future flap commands $[\delta_k, \ldots, \delta_{k+N-1}]$,
starting from the current state estimate $\hat{\tilde{\vxi}}_k$.
The key difference from LQR is that the cost is evaluated by rolling out the
\emph{full nonlinear plant} over the horizon: the optimiser sees not just the
immediate penalty on $\hat{\tilde{\vxi}}_k$, but the entire predicted trajectory
$\hat{\tilde{\vxi}}_{k+1}, \ldots, \hat{\tilde{\vxi}}_{k+N}$ as a function of
the chosen command sequence.

Crucially, the primary benefit of this rollout is \emph{not} knowledge of the
future gust.
In the realistic closed-loop setting the future gust velocity is unknown; it is
approximated crudely by a linear extrapolation of the current estimate
(Section~\ref{sec:gust_seq}).
The genuine advantage is instead \emph{multi-step planning on the nonlinear
aeroelastic response}: the latent state $\hat{z}_k$ carries aerodynamic memory
of the gust that has already entered the system, and the nonlinear rollout lets
the optimiser trace how that memory will drive $C_L$, $C_M$, $h$, and $\alpha$
over the next $N$ steps under any candidate command sequence.
The LQR gain, being a static linear map, collapses this future trajectory into a
single linearised cost-to-go and cannot distinguish between a state that will
decay naturally and one that is about to resonate.
MPC selects the command sequence that minimises the actual predicted cost over
the horizon, which automatically accounts for nonlinear aerodynamic saturation,
structural coupling, and the rate penalty $R_{\Delta u}$ jointly across all $N$
steps rather than greedily at each instant.

The cost of this improved performance is computational: instead of a single
$1\times6$ matrix--vector product, MPC requires an iterative optimisation at
every timestep.
The key methodological contribution of this chapter is to show how
\emph{reverse-mode automatic differentiation} (AD)---the same infrastructure
used to linearise the plant for LQR design in Section~\ref{sec:jacobian}---can
be applied \emph{online} to differentiate through the full $N$-step rollout and
drive an Adam gradient-descent solver, reducing the per-step wall-clock time to
a regime compatible with the $\Delta t = 0.01\,\text{s}$ sample period.
...
(identical machinery to Section~\ref{sec:lqr_arch}: leaky kinematic integrators
on $\ddot{h}$ and $\ddot{\alpha}$ to reconstruct all four structural states,
$C_L$ bisection for gust estimation, and optional EKF correction).
...
\begin{tikzpicture}[>=latex,
  ...
  \draw[->] (plant.east) -- ++(1.0,0) coordinate (mr)
            -- node[right, lbl]{$\ddot{h},\;\ddot{\alpha},\;C_L$}
...
each timestep
\begin{equation}
  \min_{\mathbf{u} \in \mathcal{U}^N}
  J_N(\mathbf{u};\,\hat{\tilde{\vxi}}_k)
  = \sum_{j=0}^{N-1}
    \Bigl[
      Q_{C_L} C_{L,k+j}^2
      + Q_{C_M} C_{M,k+j}^2
      + Q_h\, h_{k+j}^2
      + Q_\alpha\, \alpha_{k+j}^2
      + Q_{\Delta C_L}(\Delta C_{L,k+j})^2
      + R\,\delta_{k+j}^2
      + R_{\Delta u}(\delta_{k+j} - \delta_{k+j-1})^2
    \Bigr],
  \label{eq:mpc_cost}
\end{equation}
...
\begin{equation}
  k_1 = g(x_j,\, F_y,\, M_z), \quad
  k_2 = g\!\left(x_j + \tfrac{\Delta t}{2}k_1,\, F_y,\, M_z\right), \quad
  k_3 = g\!\left(x_j + \tfrac{\Delta t}{2}k_2,\, F_y,\, M_z\right), \quad
  k_4 = g(x_j + \Delta t\,k_3,\, F_y,\, M_z),
  \label{eq:rk4_slopes}
\end{equation}
...
record activations, followed by a \emph{single} backward pass, regardless of $N$.
...
(at $t \approx 0.3\,\text{s}$) compared to LQR+EKF.
This improvement stems directly from the forward gust estimate \eqref{eq:W_extrap}
fed into the horizon: the optimizer can begin deflecting the flap
\emph{before} the gust disturbance fully loads the aerodynamics, trading
the cost of early actuation against the cost of the subsequent structural
oscillation.
The LQR, lacking any preview mechanism, responds only after the aerodynamic
excitation has occurred and therefore cannot attenuate this initial transient.

In the post-gust oscillation phase ($t > 1\,\text{s}$) both controllers perform
similarly: the structural modes are nearly linear in this regime and the LQR
gain is already optimised for quadratic damping, so the advantage of the
nonlinear MPC rollout is marginal.

The MPC flap trajectory is smoother than that of the LQR in the initial phase
because the $R_{\Delta u}$ penalty \eqref{eq:mpc_cost} directly limits
actuation rate within the optimisation, whereas the LQR gain has no explicit
...
amplitude during the ramp-up phase, so the optimizer's gust preview is
imperfect.
The MPC oracle, which has exact knowledge of $W(t)$ over the horizon, achieves
a further reduction of this peak, confirming that the remaining gap between
MPC+EKF and the oracle is attributable to gust estimation error, not to any
limitation of the gradient-based solver.
The $C_M$ reduction is a by-product of flap actuation: since
$\partial C_M/\partial \delta < 0$ at trim, the $C_M$ penalty term in
\eqref{eq:mpc_cost} ($Q_{C_M} = 1$) causes the optimizer to partially trade
heave load against pitching moment in a way the LQR cannot without explicit
output augmentation.
...
Figure~\ref{fig:mpc_solve_time} shows the per-timestep solve time for MPC+EKF
over the simulation.
The first call is slower due to \texttt{@tf.function} compilation (JIT tracing
of the computation graph); subsequent steps are fully compiled and run in
approximately $[X]\,\text{ms}$ on a standard CPU, well within the
$\Delta t = 10\,\text{ms}$ budget.
By comparison, the LQR requires a single $1\times6$ matrix--vector product:
sub-microsecond, negligible.
The MPC computational overhead is thus roughly three orders of magnitude larger
...
\paragraph{Horizon $N$.}
A longer horizon allows the optimizer to plan further ahead, at the cost of a
larger computation graph and more backward-pass operations per solve.
Figure~\ref{fig:mpc_horizon} shows the peak $|\ddot{h}|$ and peak $|C_L|$
reduction (relative to open loop) as a function of $N$ for $N \in \{5, 10, 15, 20\}$.
...
preview benefit.
The solve time scales approximately linearly with $N$ (each additional step
adds one forward and one backward pass through $\Psi_j$), so $N = 10$
...
Too small a learning rate requires more iterations to converge; too large a
rate causes oscillations in the Adam trajectory and may overshoot the
(locally) optimal sequence.
For the warm-started problem the cost function is locally smooth near the
previous solution, so moderate rates ($\eta = 0.3$--$1.0$) converge reliably
within 10--20 iterations.
Figure~\ref{fig:mpc_adam} shows cost-vs-iteration curves for several $(\eta, n_{\rm iter})$
combinations: the baseline $(\eta=0.5, n_{\rm iter}=15)$ achieves $>90\%$ of
the converged cost reduction within the budget.
...
The $Q_{C_L}$ and $Q_{\Delta C_L}$ weights in \eqref{eq:mpc_cost} have no LQR
counterpart and deserve brief discussion.
Increasing $Q_{C_L}$ shifts the optimizer's priority towards aerodynamic load
...
peak displacement.
For the structural weight choices this chapter inherits the same
$Q_h / Q_\alpha / R$ normalisations as \eqref{eq:weights}; their trade-off
analysis applies identically (see Section~\ref{sec:sensitivity}).
