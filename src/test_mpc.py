...
Q_hd  = 0.0
Q_ad  = 0.0
Q_dCL = 1.0 / 0.005**2    # same scale as Q_CL*C_L_rms^2 ≈ 1
R_mpc = 1.0 / 2.0**2
R_du  = 2.0 / 2.0**2
...
mpc_tf = MPCController(aero_model, U_INF, DT,
                       ...
                       use_tf_solver=True, Q_dCL=Q_dCL)
...
print(f"{'':12s}  {'Baseline':>10s}  {'MPC-dCL':>10s}  {'Red%':>7s}")
...
TFMPC = dict(color='tomato',    lw=1.5, alpha=0.85, label='MPC dCL/dt')
...
fig1.suptitle('Structural State  —  Baseline vs MPC dCL/dt', fontsize=13)
axes[0,0].plot(t_b, res_b['h'],  **BL);  axes[0,0].plot(t_tf, res_tf['h'],  **TFMPC)
fmt(axes[0,0], 'h [m]', 'Heave displacement')
axes[0,1].plot(t_b, res_b['hd'], **BL);  axes[0,1].plot(t_tf, res_tf['hd'], **TFMPC)
...
fig2.suptitle('Structural Accelerations  —  Baseline vs MPC dCL/dt', fontsize=13)
axes[0].plot(t_b, res_b['h_ddot'],  **BL); axes[0].plot(t_tf, res_tf['h_ddot'], **TFMPC)
...
fig4.suptitle('Control Input  —  MPC dCL/dt', fontsize=13)
...
fig5.suptitle('Aerodynamic Coefficients  —  Baseline vs MPC dCL/dt', fontsize=13)
axes[0].plot(t_b, res_b['C_L'],  **BL); axes[0].plot(t_tf, res_tf['C_L'], **TFMPC)
fmt(axes[0], '$C_L$', 'Lift coefficient')
axes[1].plot(t_b, res_b['C_M'],  **BL); axes[1].plot(t_tf, res_tf['C_M'], **TFMPC)
