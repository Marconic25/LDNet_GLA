...
def interface_power(t_win, Fy_arr, Mz_arr, h_arr, alpha_arr):
    ...
    P_arr  = -Fy_arr * hd_arr + Mz_arr * ad_arr  # heave EOM forcing is -Fy (see cosim_driver docstring)
...
def run_energy_analysis(
    ...
    if hd_full is None or ad_full is None:
        # fall back to finite-difference velocities
        P_full, hd_full, ad_full = interface_power(
            t_full, Fy_full, Mz_full, h_traj_full, alpha_traj_full
        )
    else:
        # integrator velocities: exact, no boundary artefacts from
        # np.gradient over duplicated window-boundary times
        P_full = -Fy_full * hd_full + Mz_full * ad_full
