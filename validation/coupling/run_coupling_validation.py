...
def replay_trajectory(sorted_keys, windows, win_range=None):
    ...
    # ICs: match the actual run start (checkpoint warm start), not rest
    h0, hd0, a0, ad0 = 0.0, 0.0, 0.0, 0.0
    _t0_file = ROOT_DIR / "cosim_state_t0.json"
    if _t0_file.exists():
        import json as _json
        with open(_t0_file) as _f:
            _s = _json.load(_f)
        h0, hd0, a0, ad0 = _s["h"], _s["hd"], _s["a"], _s["ad"]
        print(f"  ICs from cosim_state_t0.json: h={h0*1000:.3f}mm  alpha={np.degrees(a0):.4f}deg")
    ...
    for t_start in keys:
        ...
        # Integrate
        h_f, hd_f, a_f, ad_f, h_arr, _hd_arr, alpha_arr, _ad_arr = integrate_structural(
        ...
        hd_parts.append(_hd_arr[:n_out])
        ad_parts.append(_ad_arr[:n_out])
    ...
    hd_full   = np.concatenate(hd_parts)
    ad_full   = np.concatenate(ad_parts)
    ...
    return (t_windows, Fy_traj, Mz_traj,
            ...
            h_full, alpha_full, hd_full, ad_full)
...
def main():
    ...
    (t_windows, Fy_traj, Mz_traj,
     ...
     h_full, alpha_full, hd_full, ad_full) = replay_trajectory(sorted_keys, windows, win_range)
    ...
    if not args.skip_energy:
        ...
        eb_summary = eb_mod.run_energy_analysis(
            h_full, alpha_full,
            Fy_traj, Mz_traj, t_windows,
            hd_full=hd_full, ad_full=ad_full
        )
    else:
