...
MESH_LEVELS = {
    "M1": dict(surf_min=3, surf_max=3, box=1, gap=2, layers=1,  first_layer=0.40, exp_ratio=1.20),
    "M2": dict(surf_min=4, surf_max=4, box=2, gap=3, layers=2,  first_layer=0.30, exp_ratio=1.20),
...
def patch_snappy(src: Path, dst: Path, p: dict):
    ...
    # Every pattern must match: a silent no-op here produces a mesh with the
    # template levels (all ~340k cells) and invalidates the whole GCI family.
    def _sub(pattern, repl, s, what):
        s, n = re.subn(pattern, repl, s)
        if n == 0:
            raise RuntimeError(f"patch_snappy: pattern for {what} did not match template")
        return s

    # Surface refinement levels
    txt = _sub(r"(wing_main\s*\{\s*level\s*\(\s*)(\d+\s+\d+)(\s*\)\s*;)",
               rf"\g<1>{sl_min} {sl_max}\g<3>", txt, "wing_main surface level")
    txt = _sub(r"(flap\s*\{\s*level\s*\(\s*)(\d+\s+\d+)(\s*\)\s*;)",
               rf"\g<1>{sl_min} {sl_max}\g<3>", txt, "flap surface level")

    # Feature edge levels
    txt = _sub(r'(file\s+"wing_main\.eMesh"\s*;\s*level\s+)(\d+)(\s*;)',
               rf"\g<1>{sl_max}\g<3>", txt, "wing_main feature level")
    txt = _sub(r'(file\s+"flap\.eMesh"\s*;\s*level\s+)(\d+)(\s*;)',
               rf"\g<1>{sl_max}\g<3>", txt, "flap feature level")

    # Refinement box level
    txt = _sub(r"(refinementBox\s*\{\s*mode\s+inside;\s*levels\s+\(\s*\(\s*1e\+?15\s+)(\d+)(\s*\)\s*\))",
               rf"\g<1>{box}\g<3>", txt, "refinementBox level")

    # Gap region level
    txt = _sub(r"(gapRegion\s*\{\s*mode\s+inside;\s*levels\s+\(\s*\(\s*1e\+?15\s+)(\d+)(\s*\)\s*\))",
               rf"\g<1>{gap}\g<3>", txt, "gapRegion level")
...
def main():
    ...
    if args.results_json:
        ...
        for name, params in MESH_LEVELS.items():
            case_dir = STUDY_DIR / name
            # Reuse results.json entry when params match (case dirs may be gone)
            if name in cached and cached[name].get("params") == dict(params):
                print(f"  {name}: cached in results.json (params match), reusing...")
